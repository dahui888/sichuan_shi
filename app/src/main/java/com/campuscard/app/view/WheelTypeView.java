 ...
 * 滚动控件
 */
public class WheelTypeView extends View {
     ...
     * 设置数据
     */
    private ArrayList<ClassEntity> dataList = new ArrayList<>();
    /**
     * 按下的Y坐标
     */
    private int downY;
    ...
    public WheelTypeView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
        initData();
    }

    public WheelTypeView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
        initData();
    }

    public WheelTypeView(Context context) {
        super(context);
        initData();
    }
...
//                Log.d(TAG,"time="+time+",y - downY="+(y - downY));
    ...
    private void initData() {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            itemObject.id = i;
            itemObject.setItemText(dataList.get(i).getName());
            itemObject.setWork(dataList.get(i));
            itemObject.x = 0;
...
//        }
    ...
    private void drawMask(Canvas canvas) {
        LinearGradient lg = new LinearGradient(0, 0, 0, itemNumber / 2 * unitHeight, 0xffffffff,
                0xc0ffffff, Shader.TileMode.MIRROR);
        ...
        LinearGradient lg2 = new LinearGradient(0, controlHeight - unitHeight,
                0, itemNumber / 2 * unitHeight, 0xc0ffffff, 0xffffffff, Shader.TileMode.MIRROR);
    ...
    private void onEndSelecting(final ItemObject toShowItem) {
        if (onSelectListener != null) {
            callbackHandler.post(new Runnable() {
                @Override
                public void run() {
                    onSelectListener.endSelect(toShowItem.id, toShowItem.getWork());
                }
            });
        }
    }
...
//        Log.e(TAG,"---------------------------------------------------------------------------------------------------------");
        ...
        if (onSelectListener != null && toShowItems[itemNumber / 2] != null) {
            callbackHandler.post(new Runnable() {
                @Override
                public void run() {
                    onSelectListener.selecting(toShowItems[itemNumber / 2].id, toShowItems[itemNumber / 2].getWork());
                }
            });
        }
...
//        Log.d(TAG,"slowMove end move="+move);
     ...
     * @param data 数据集
     */
    public void setData(ArrayList<ClassEntity> data) {
        this.dataList = data;
        initData();
    }
     ...
     * @param data 数据集
     */
    public void refreshData(ArrayList<ClassEntity> data) {
        setData(data);
        findItemsToShow();
        invalidate();
    }
...
//            if (y + move > controlHeight || ((float)y + (float)move + (float)unitHeight / 2 + (float)textRect.height() / 2f) < 0)
        ...
        public void setWork(ClassEntity area) {
            this.area = area;
        }
    }
    ...
    public interface OnSelectListener {
         ...
         *
         * @param id
         * @param areaProvEntity
         */
        void endSelect(int id, ClassEntity areaProvEntity);
         ...
         * @param areaProvEntity
         */
        void selecting(int id, ClassEntity areaProvEntity);
